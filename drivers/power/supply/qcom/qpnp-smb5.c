...
static bool get_prop_batt_fg_soc_changed(struct smb_charger *chip)
{
	int rc;
	union power_supply_propval soc_changed;

	rc = smblib_get_prop_from_bms(chip, POWER_SUPPLY_PROP_CALIBRATE, &soc_changed);
	if (rc) {
		pr_err("Couldn't get soc changed from bms rc = %d\n",rc);
		soc_changed.intval = 1;
	}
	return soc_changed.intval;
}

static int get_prop_batt_capacity(struct smb_charger *chip)
{
	int capacity;
	int capacity_int = 0;
	...
	if ((capacity_int == 100) && (chip->last_soc == 99)) {
		...
		pr_debug("lvchen external voltage is %d, chip->last_soc=%d\n", voltage_int, chip->last_soc);
	}
	goto return_soc;

internal_soc:
	if (soc_count < 2) {
		capacity_int = chip->last_soc;
		soc_count++;
		g_power_is_couloMeter = 1;
		pr_debug("internal soc: cm last_soc=%d, soc_count=%d\n", chip->last_soc, soc_count);
	}
	else if (soc_count == 2) {
		rc = smblib_get_prop_batt_capacity(chip, &pval);
		capacity_int = pval.intval;
		if (rc) {
			pr_err("Couldn't get capacity from QG, rc = %d\n", rc);
			capacity_int = DEFAULT_BATT_CAPACITY;
		}
		pr_debug("%s: get internal capcity is %d\n", __func__, capacity_int);
	}
	...
	if ((chip->last_soc == 50) && (abs(capacity_int - chip->last_soc) > 1))
	 ...
	 if((capacity_int == 100) && (chip->last_soc == 99)) {
		smblib_get_prop_from_bms(chip, POWER_SUPPLY_PROP_VOLTAGE_NOW, &pval);
		voltage_int = pval.intval;
		voltage_int /= 1000;
		pr_debug("%s: internal voltage is %d\n", __func__, voltage_int);
	 }

return_soc:
	soc_count = 0;
	if (chip->last_soc < 0) {
		pr_debug("%s: the first get capacity = %d\n", __func__, capacity_int);
		chip->last_soc = capacity_int;
	...
	if (ktime_to_ms(ktime_sub(now, chip->last_time)) < 2000) {
		pr_debug("%s: ktime_to_ms return %d, time less than 2s\n", __func__, chip->last_soc );
		return chip->last_soc;
	}
	...
	if (abs(capacity_int - chip->last_soc) > 1) {
		...
		smblib_get_prop_from_bms(chip, POWER_SUPPLY_PROP_CURRENT_NOW, &pval);
		current_int = pval.intval;
		pr_debug("%s: capacity_int=%d, chg_status=%d, current_int=%d",
				 __func__, capacity_int, chg_status, current_int);

		if ((capacity_int - chip->last_soc > 1) && (chg_status != POWER_SUPPLY_STATUS_CHARGING)
		&& (chg_status != POWER_SUPPLY_STATUS_FULL)) {
			pr_debug("%s: return %d,chg_status no charging no full\n", __func__, chip->last_soc);
			return chip->last_soc;
		}
		if ((chip->last_soc - capacity_int > 1) && (chg_status == POWER_SUPPLY_STATUS_CHARGING)
		&& (current_int < 0)) {
			pr_debug("%s: return %d,chg_status real charging", __func__, chip->last_soc);
			return chip->last_soc;
		}
		if (get_prop_batt_fg_soc_changed(chip) == 0) {
			pr_debug("%s: internal fg_changed no\n", __func__);
			return chip->last_soc;
		}
	...
	if ((capacity_int == 100) && (chip->last_soc == 99) && (voltage_int < 4360)) {
		pr_debug("%s: charging No full keep capacity to 99 percent!\n", __func__);
		return chip->last_soc;
	}
	...
	if ((capacity_int == 99) && (chip->last_soc == 100)) {
		...
		if(chg_status == POWER_SUPPLY_STATUS_FULL) {
			pr_debug("%s: charging full keep capacity to 100 percent!\n", __func__);
			return chip->last_soc;
		}
	}

	chip->last_soc = capacity_int;
	pr_debug("%s: return capcity is %d\n", __func__, capacity_int);
	return capacity_int;
}
#endif
...
#ifdef CONFIG_COULO_METER
		if (cm_iic_client)
			val->intval = get_prop_batt_capacity(chg);
		else
#endif
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	if (chg->connector_pull_up != -EINVAL) {
		...
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't configure BAT_THERM pull-up rc=%d\n", rc);
			return rc;
		}
	}
...
static int batt_proc_show(struct seq_file *seq, void *v)
{
	int id1;

	id1 = cm_get_ID1();
	if (id1 == 0x21)
		seq_puts(seq, "battery_name:FMT\n");
	else
		seq_puts(seq, "battery_name:JBY\n");
	return 0;
}

static int batt_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, batt_proc_show, inode->i_private);
}

static const struct file_operations batt_manuf_info = {
	.open = batt_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
static int smb5_probe(struct platform_device *pdev)
{
	...
	if (!proc_create("app_info", 0666, NULL, &batt_manuf_info))
		pr_err("proc create batt_manuf_info failed\n");
