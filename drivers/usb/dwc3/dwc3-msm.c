...
#if USBID_GPIO
#define ID_T_HUB_WITH_POWER 900*1000 	//0.4v~0.9v, normally 0.544v~0.68v
#define ID_T_HUB_WITHOUT_POWER 21*1000	//0~0.4v, normally it's around 0.021V
#define T_HUB_CHARGING_CURRENT 500	//500mA
#define T_HUB_DETECT_VOL 250*1000	//normally it's around 0.36V
#define T_HUB_WORK_DELAY_MS 500    //unit  ms
#endif
/* XHCI registers */
...
#if USBID_GPIO
enum t_hub_adcid_state {
	USBID_T_HUB_WITHOUT_POWER = 0,
	USBID_T_HUB_WITH_POWER,
	USBID_HIGH,
	USBID_INVAIL = USBID_HIGH,
};
#endif
...
#if USBID_GPIO
struct dwc3_wakeup_source {
	struct wakeup_source	source;
	bool	enabled;
};

static void dwc3_stay_awake(struct dwc3_wakeup_source *source)
{
	__pm_stay_awake(&source->source);
	source->enabled = true;
	pr_debug("enabled wakeup source %s\n", source->source.name);
}

static void dwc3_relax(struct dwc3_wakeup_source *source)
{
	__pm_relax(&source->source);
	source->enabled = false;
	pr_debug("disabled wakeup source %s\n", source->source.name);
}
#endif
...
#if USBID_GPIO
	struct delayed_work 	usbid_status_work;
	struct delayed_work 	t_hub_detect_work;
	struct qpnp_vadc_chip 	*usbid_vadc_dev;
	int			usbid_gpio;
	int			usbid_irq;
	bool			t_hub_power_on;
	struct dwc3_wakeup_source usbid_wakeup_source;
#endif
};
...
#if USBID_GPIO
static void usbid_status_w(struct work_struct *w);
static irqreturn_t usbid_changed_irq_handler(int irq, void *data);
#endif
...
#if USBID_GPIO
static enum t_hub_adcid_state t_hub_read_adc_id_state(struct dwc3_msm *mdwc)
{
	int rc = 0;
	struct qpnp_vadc_result results;

	if (IS_ERR_OR_NULL(mdwc->usbid_vadc_dev)) {
		mdwc->usbid_vadc_dev = qpnp_get_vadc(mdwc->dev, "usbid");
		if (IS_ERR(mdwc->usbid_vadc_dev)) {
			dev_dbg(mdwc->dev, "[t-hub] Unable to get usbid_vadc_dev.\n");
			return USBID_INVAIL;
		}
	}
	/*******P_MUX2_1_1 is chanel 0x11********/
	rc = qpnp_vadc_read(mdwc->usbid_vadc_dev, P_MUX2_1_1, &results);
	if (rc) {
		dev_dbg(mdwc->dev,"[t-hub] Unable to read usbid voltage rc=%d\n", rc);
		return USBID_INVAIL;
	} else {
		if ((int)results.physical < ID_T_HUB_WITHOUT_POWER)
			return USBID_T_HUB_WITHOUT_POWER;
		else if ((int)results.physical < ID_T_HUB_WITH_POWER)
			return USBID_T_HUB_WITH_POWER;
		else
			return USBID_HIGH;
	}
...
static void dwc3_set_id_ground(struct dwc3_msm *mdwc)
{
	union power_supply_propval pval = {0};

	mdwc->id_state = DWC3_ID_GROUND;
	if (!test_and_clear_bit(ID, &mdwc->inputs))
		dev_dbg(mdwc->dev,"[t-hub] ID clear\n");
...
static void dwc3_set_id_float(struct dwc3_msm *mdwc)
{
	union power_supply_propval pval = {0};

	mdwc->id_state = DWC3_ID_FLOAT;
	if (test_and_set_bit(ID, &mdwc->inputs))
		dev_dbg(mdwc->dev,"[t-hub] ID set\n");
	pval.intval = 0;
	mdwc->t_hub_power_on = false;
	power_supply_set_property(mdwc->usb_psy,
		POWER_SUPPLY_PROP_USB_OTG, &pval);
...
static int g_charging_flag = 0;
static void t_hub_detect_w(struct work_struct *w)
{
	struct dwc3_msm *mdwc = container_of(w, struct dwc3_msm,
				t_hub_detect_work.work);
	struct qpnp_vadc_result results;
	static bool usb_insert = false;
	int rc = 0;
	...
	if ((int)results.physical > T_HUB_DETECT_VOL)
		usb_insert = true;
	else
		usb_insert = false;
	...
	if (usb_insert) {
		if (!g_charging_flag) {
			dev_dbg(mdwc->dev,"[t-hub] preparing charging\n");
			dwc3_set_id_float(mdwc);
			msleep(400);
			dwc3_set_id_ground(mdwc);
			mdwc->t_hub_power_on = true;
			g_charging_flag = 1;
		} else
		    dev_dbg(mdwc->dev,"[t-hub] do charging\n");
	} else
		g_charging_flag = 0;

	schedule_delayed_work(&mdwc->t_hub_detect_work,
				msecs_to_jiffies(T_HUB_WORK_DELAY_MS));
}

static void usbid_status_w(struct work_struct *w)
{
	struct dwc3_msm *mdwc = container_of(w, struct dwc3_msm,
					usbid_status_work.work);
	enum t_hub_adcid_state adc_id_state;
	union power_supply_propval pval = {0};

	adc_id_state = t_hub_read_adc_id_state(mdwc);

	switch (adc_id_state) {
	case USBID_T_HUB_WITHOUT_POWER:
	case USBID_T_HUB_WITH_POWER:
		if (!mdwc->usbid_wakeup_source.enabled)
			dwc3_stay_awake(&mdwc->usbid_wakeup_source);
	...
	default:
		...
		dwc3_set_id_float(mdwc);
		if (mdwc->usbid_wakeup_source.enabled)
			dwc3_relax(&mdwc->usbid_wakeup_source);

		cancel_delayed_work_sync(&mdwc->t_hub_detect_work);
		break;
	}
...
static irqreturn_t usbid_changed_irq_handler(int irq, void *data)
{
	...
	schedule_delayed_work( &mdwc->usbid_status_work,
...
#if USBID_GPIO
	INIT_DELAYED_WORK(&mdwc->usbid_status_work, usbid_status_w);
	INIT_DELAYED_WORK(&mdwc->t_hub_detect_work, t_hub_detect_w);
#endif
...
#if USBID_GPIO
	mdwc->usbid_gpio = of_get_named_gpio(node, "usbid-gpio", 0);

	if (gpio_is_valid(mdwc->usbid_gpio)
		&& (!devm_gpio_request(&pdev->dev, mdwc->usbid_gpio,
			"usbid_gpio"))) {
		gpio_direction_input(mdwc->usbid_gpio);
		if (!gpio_get_value(mdwc->usbid_gpio)) {
			schedule_delayed_work(&mdwc->usbid_status_work, msecs_to_jiffies(3000));
		}
		mdwc->usbid_irq = gpio_to_irq(mdwc->usbid_gpio);

		ret = devm_request_threaded_irq(mdwc->dev, mdwc->usbid_irq, NULL,
				usbid_changed_irq_handler,
				IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING
				| IRQF_ONESHOT,
				"usbid_irq", mdwc);
		if (ret < 0)
			dev_err(&pdev->dev, "Couldn't request usbid_irq %d\n", mdwc->usbid_irq);
		else
			enable_irq_wake(mdwc->usbid_irq);
	}
#endif
...
#if USBID_GPIO
	wakeup_source_init(&mdwc->usbid_wakeup_source.source, "usbid");
#endif
...
#if USBID_GPIO
	cancel_delayed_work_sync(&mdwc->usbid_status_work);
	cancel_delayed_work_sync(&mdwc->t_hub_detect_work);
	wakeup_source_trash(&mdwc->usbid_wakeup_source.source);
#endif
	if (mdwc->hs_phy)
...
#if USBID_GPIO
		if (!IS_ERR_OR_NULL(mdwc->vbus_reg) && !mdwc->vbus_active
				&& !regulator_is_enabled(mdwc->vbus_reg))
#else
		if (!IS_ERR_OR_NULL(mdwc->vbus_reg))
#endif
...
static void dwc3_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_A_IDLE:
		dev_err(mdwc->dev, "mark otg_state_a_dle\n");
...
#if USBID_GPIO
			if (mdwc->t_hub_power_on) {
				pval.intval = 1000 * T_HUB_CHARGING_CURRENT;
				ret = power_supply_set_property(mdwc->usb_psy,
					POWER_SUPPLY_PROP_SDP_CURRENT_MAX, &pval);
				if (ret)
					dev_dbg(mdwc->dev, "power supply error when setting property\n");
				else
					mdwc->max_power = T_HUB_CHARGING_CURRENT;
			}
#endif
...
#if USBID_GPIO
			if (!mdwc->vbus_active && mdwc->t_hub_power_on) {
				if (USBID_T_HUB_WITH_POWER == t_hub_read_adc_id_state(mdwc))
					mdwc->t_hub_power_on = true;
				else
					mdwc->t_hub_power_on = false;
				dwc3_otg_start_host(mdwc, 0);
