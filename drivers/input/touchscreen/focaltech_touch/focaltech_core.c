...
extern int hq_get_ctp_module_info(char *buf);
/*****************************************************************************
...
#endif
/****************************************************************************
 * Dsi VSP/VSN control fucntion to power off dsi panle voltage
****************************************************************************/
extern int hq_dsi_power_on_vspn(void);
extern u8 hq_get_gesture_mode(void);
/*****************************************************************************
                                ...
                                unsigned long event, void *data)
{
	...
	if (*blank == FB_BLANK_UNBLANK)
                ...
                FTS_DEBUG("%s:power off dsi vspn!!!\n",__func__);
                //gesture_mode is 0 means that disable the gesture function.
                if (hq_get_gesture_mode() == 1) {
                        hq_dsi_power_on_vspn();
                }
        }
...
#endif
static char hq_ctp_module_info[128];

int fts_hq_info(void)
{
	int ret = -1;
	ret = hq_get_ctp_module_info(hq_ctp_module_info);
	if(ret <= 0) {
		hq_regiser_hw_info(HWID_CTP,"CTP Module info is null");
	} else {
		hq_regiser_hw_info(HWID_CTP,hq_ctp_module_info);
	}
	return ret;
}
