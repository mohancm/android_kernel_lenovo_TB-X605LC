...
int hq_get_ctp_module_info(char *buf)
{
	int count = 0;
	struct fts_ts_data *ts_data = fts_data;
	struct i2c_client *client = ts_data->client;
	...
	count += snprintf(buf + count, 128, "Driver Ver:%s\n", FTS_DRIVER_VERSION);
	count += snprintf(buf + count, 128, "FocalTech IC ID:0x%02x%02x\n",
		ts_data->ic_info.ids.chip_idh, ts_data->ic_info.ids.chip_idl);
	count += snprintf(buf + count, 128, "Module Vendor:BOE\n");
#if FTS_ESDCHECK_EN
	fts_esdcheck_proc_busy(1);
#endif
	if (fts_i2c_read_reg(client, FTS_REG_FW_VER, &fwver) < 0) {
		count += snprintf(buf, PAGE_SIZE, "I2c transfer error!\n");
	}
#if FTS_ESDCHECK_EN
	fts_esdcheck_proc_busy(0);
#endif
	if ((fwver == 0xFF) || (fwver == 0x00))
		count += snprintf(buf + count, 128, "get tp fw version fail!\n");
	else
		count += snprintf(buf + count, 128, "FW version:%02x", fwver);
