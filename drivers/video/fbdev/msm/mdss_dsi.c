...
/*enable touchpanel gesture function  on ft8201 drives*/
static struct mdss_dsi_ctrl_pdata *temp_ctrl_pdata = NULL;
static int mdss_dsi_panel_power_off(struct mdss_panel_data *pdata)
{
	...
	if (mdss_dsi_pinctrl_set_state(ctrl_pdata, false))
        ...
        /*enable touchpanel gesture function  on ft8201 drives*/
        temp_ctrl_pdata = ctrl_pdata;
        /*enable touchpanel gesture function  on ft8201 drives*/
        //enable ft8201 touchpanel gesture feather
...
int hq_dsi_power_on_vspn(void)
{
        int ret = 0;
        if(temp_ctrl_pdata == NULL) {
                pr_err("%s:temp_ctrl_pdata is null \n",__func__);
                return -1;
        }
        pr_debug("%s:VSP/VSN is power off \n",__func__);
        ret = msm_mdss_enable_vreg(
		temp_ctrl_pdata->panel_power_data.vreg_config,
		temp_ctrl_pdata->panel_power_data.num_vreg, 1);
	if (ret) {
		pr_err("%s: failed to disable vregs for %s\n",
			__func__, __mdss_dsi_pm_name(DSI_PANEL_PM));
                return -1;
...
char hq_panel_name[MDSS_MAX_PANEL_LEN] = "";
		...
		struct platform_device *pdev, char *panel_cfg)
{
	...
	if (!len) {
		...
		if(strstr(panel_name,"ft8201"))
			strlcpy(hq_panel_name,"FT8201",sizeof(hq_panel_name));
		else
			strlcpy(hq_panel_name,"UNKNOW",sizeof(hq_panel_name));
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	hq_regiser_hw_info(HWID_LCM,hq_panel_name);
	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->touch_reset_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "focaltech,reset-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->touch_reset_gpio))
