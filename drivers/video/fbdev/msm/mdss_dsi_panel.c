...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		/*Add Special power on consequence for focal ft8201*/
		msm_mdss_enable_vreg(ctrl_pdata->panel_power_data.vreg_config,ctrl_pdata->panel_power_data.num_vreg, 1);
		usleep_range(22*1000,(22*1000)+10);

		gpio_direction_output(ctrl_pdata->touch_reset_gpio,0);
		usleep_range(5*1000,(5*1000)+10);
		gpio_set_value((ctrl_pdata->rst_gpio),0);
		usleep_range(5*1000,(5*1000)+10);

		gpio_direction_output(ctrl_pdata->touch_reset_gpio,1);
		...
		gpio_set_value((ctrl_pdata->touch_reset_gpio),1);
