...
int msm_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	struct msm_camera_slave_info *slave_info;
	const char *sensor_name;
        uint16_t flag = 0, supplier = 0, ret = 0;
	...
	if (msm_sensor_id_by_mask(s_ctrl, chipid) != slave_info->sensor_id) {
        ...
        /*add for read supplider id begin
        register refer to s5k4h7yx_qtech_front_eeprom.h or s5k4h7yx_ofilm_front_eeprom.h mem setting*/
        if (strcmp(sensor_name,"s5k4h7yx_ofilm_front") == 0 ||
            strcmp(sensor_name,"s5k4h7yx_qtech_front") == 0) {
            ...
            if ((flag & 0xc0) == 0x40) {
                ret = sensor_i2c_client->i2c_func_tbl->i2c_read(
                    ...
                    &supplier, MSM_CAMERA_I2C_BYTE_DATA);
            }else{
                ret = sensor_i2c_client->i2c_func_tbl->i2c_write(
                    sensor_i2c_client, 0x0a02,
                    0x16, MSM_CAMERA_I2C_BYTE_DATA);
                msleep(1);
                ret = sensor_i2c_client->i2c_func_tbl->i2c_write(
                    sensor_i2c_client, 0x0a00,
                    0x01, MSM_CAMERA_I2C_BYTE_DATA);
            ...
            if ((flag & 0xc0) == 0x40) {
                ...
                if ((flag & 0xc0) == 0x40) {
                    ...
                    pr_err("read supplier id failed,can not get valid group");
                }
            }
        }
            ret = sensor_i2c_client->i2c_func_tbl->i2c_write(
            ...
            ret = sensor_i2c_client->i2c_func_tbl->i2c_write(
                    ...
                    0x00, MSM_CAMERA_I2C_BYTE_DATA);
            msleep(1);
            if ((strcmp(sensor_name,"s5k4h7yx_qtech_front") == 0 && supplier == 0x06) ||
                (strcmp(sensor_name,"s5k4h7yx_ofilm_front") == 0 && supplier == 0x07)) {
                pr_err("match supplier id succeed sensor name=%s supplier id=%d",sensor_name,supplier);
            }else{
                pr_err("match supplier id failed supplier id=%d",supplier);
                return -ENODEV;
            }
        }
        //add for read supplider id end
	return rc;
}
